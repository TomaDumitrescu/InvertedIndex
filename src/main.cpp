...
void *reduce_op(void *arg)
{
    ...
    for (int i = r_arg->start; i < r_arg->end; ++i) {
        ...
        for (const auto& item: r_arg->local_maps[i]) {
            r_arg->global_maps[item.first[0] - 'a'][item.first].insert(item.second.begin(),
                                                                            item.second.end());
        }
    ...
    for (int i = r_arg->start_2; i < r_arg->end_2; ++i) {
        // Avoid copying the dictionary by using a reference
        ...
        char file[6];
        strcpy(file + 1, ".txt\0");
        for (int j = r_arg->start_2; j < r_arg->end_2; j++) {
            file[0] = j + 'a';
            ofstream out(file);

            for (const auto& word: r_arg->global_multisets[j]) {
                const auto& id_list = r_arg->global_maps[j][word];
                out << word << ":[";
...
int main(int argc, char **argv)
{
    ...
    for (id = 0; id < reducers; id++) {
        ...
        red_args[id].end = ((id + 1) * 1.0 * mappers / reducers < 1.0 * mappers)?
                            (id + 1) * 1.0 * mappers / reducers : mappers;
        red_args[id].end_2 = ((id + 1) * 1.0 * 26 / reducers < 26.0)? (id + 1) * 1.0 * 26 / reducers : 26;
    }
    ...
    unordered_map<string, set<int>> thread_maps[mappers];
    unordered_map<string, set<int>> global_maps[NUM_LETTERS];
    multiset<string, function<bool(const string&, const string&)>> global_multisets[NUM_LETTERS];
